...
initial_values_var = lambda: th.rand(hidden_dim)*6
...
for j in range(epochs):
    ...
    if j % 100 == 0:
        with th.no_grad():
            ...
            for idx in product(*[range(i) for i in scales_test.shape]):
                ax = cast(plt.Axes, slice_(axs, idx))
                ax.clear()
                scale = cast(th.Tensor, slice_(scales_test, idx))
                if th.isnan(scale):
                    act_params = th.cat([th.sqrt(a.variance) for a in model.activations]).flatten() #type: ignore
