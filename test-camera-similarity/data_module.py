...
from typing import Any, Callable, Iterator, Optional, Literal, Union, cast
...
class QualitySchedule:
    def __init__(self,
                 start_quality: float,
                 max_quality_step: int,):
        self.start_quality = start_quality
        self.max_quality_step = max_quality_step
        self.step = 0
    
    def __call__(self):
        out = min(1., self.start_quality + self.step * (1-self.start_quality) / self.max_quality_step)
        self.step += 1
        return out
...
class ImagePoseDataModule(pl.LightningDataModule):
    ...
    def __init__(
        ...
        quality_schedule: QualitySchedule = QualitySchedule(1., 1,),
    ...
    def _get_dataset(self, purpose: Literal["train", "val", "test"]) -> ImagePoseDataset:
        ...
        """
        if purpose == "train":
            self.quality = self.quality_schedule()

        dataset = ImagePoseDataset(
            image_width=int(self.image_width*self.quality), #type: ignore
            image_height=int(self.image_height*self.quality), #type: ignore
